...
This project is the **server**. The UnityMCP server may have both the server and
...
next call and routing silently falls back to another instance (e.g. the client).

**Instead, pass `unity_instance` explicitly on EVERY UnityMCP tool call** in this
project, targeting the server:

1. Resolve the server id by name: read `mcpforunity://instances`, find the
   instance whose `name` is `LeagueOfPhysical-Server`, take its full `id`
   (`Name@hash`). The hash can change.
2. Pass that id as the `unity_instance` argument on each tool call
   (e.g. `read_console(..., unity_instance="LeagueOfPhysical-Server@<hash>")`).
...
Never operate against the client instance from this project unless the user
explicitly asks.
